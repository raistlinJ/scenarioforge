...
- **core-daemon check/start/tunnel**: "Validate Core VM" (Web UI) and `--generate` (CLI) both check whether `CORE_HOST:CORE_PORT` is reachable. If not, they SSH into `CORE_SSH_HOST` and either offer to run `systemctl start core-daemon` (it's simply down there) or open a local SSH port-forward bound to `CORE_HOST:CORE_PORT` itself (it's already running but only listens on the VM's loopback interface). The forward is a plain local listener, so the `scenarioforge.cli` subprocess a run then spawns connects to `CORE_HOST:CORE_PORT` exactly as always and rides it transparently — no separate `ssh -L` needed. See `core_daemon.py`.
