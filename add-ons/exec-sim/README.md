...
- **core-daemon check/start**: "Validate Core VM" (Web UI) and `--generate` (CLI) both check whether `CORE_HOST:CORE_PORT` is reachable. If not, they SSH into `CORE_SSH_HOST` to see whether `core-daemon` is simply down there (offering to run `systemctl start core-daemon`) versus already running but unreachable from here, in which case an SSH tunnel is needed instead (see `core_daemon.py`).
