...
def _validate_inject_output_paths(out_dir: Path, inject_files: list[str]) -> None:
    """Fail when an inject entry resolves to an absolute path inside the output dir.

    ``outputs.json`` values must be relative to ``/outputs``. An absolute
    ``/outputs/...`` value is not an injectable path, so inject expansion drops
    the entry: the run then succeeds having staged nothing, and the artifact
    never reaches a participant. Report it here instead of losing it silently.
    """
    ...
    try:
        out_dir_prefix = str(out_dir.resolve()).replace('\\', '/').rstrip('/') + '/'
    except Exception:
        out_dir_prefix = ''

    def _is_output_dir_absolute(value: str) -> bool:
        text = str(value or '').strip().replace('\\', '/')
        if not text.startswith('/'):
            return False
        return text.startswith('/outputs/') or bool(out_dir_prefix and text.startswith(out_dir_prefix))
    ...
    for raw in inject_files or []:
        ...
        for item in values:
            ...
            if text and _is_output_dir_absolute(text):
                offenders.append(f'{key}={text}')

    if offenders:
        summary = ', '.join(offenders[:8]) + ('...' if len(offenders) > 8 else '')
        print(f"[inject_files] error: absolute output paths are not injectable: {summary}")
        raise ValueError(
            'inject_files validation failed: outputs.json values for injected artifacts must be '
            f'relative to /outputs (for example artifacts/my_file), got {summary}'
        )
...
def main() -> int:
    ...
    if isinstance(compose, dict):
        ...
        _validate_inject_output_paths(out_dir, expanded_inject)
