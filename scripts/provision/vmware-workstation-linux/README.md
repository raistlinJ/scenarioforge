...
bash scripts/provision/vmware-workstation-linux/install-scenarioforge-lab.sh --reinstall participant --dry-run
bash scripts/provision/vmware-workstation-linux/install-scenarioforge-lab.sh --reinstall participant

# Rebuild all three VMs.
bash scripts/provision/vmware-workstation-linux/install-scenarioforge-lab.sh --reinstall all
```
...
`${XDG_STATE_HOME:-$HOME/.local/state}/scenarioforge-vmware-lab`; if you originally set `SCENARIOFORGE_VMWARE_STATE_DIR`, set it to
