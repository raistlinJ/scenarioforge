...
try {
    foreach ($target in @('core', 'app', 'participant', 'all')) {
        foreach ($mode in @('run', 'preview', 'bad-cache', 'build-failure', 'unowned', 'still-running')) {
            & {
                ...
                function Invoke-ReinstallBuild {
                    param($Config, $RequestFile, [switch]$CheckCache, [switch]$PromptMissing)
                    if ($CheckCache) {
                ...
                $failed = $false
                try { Reinstall-LabVMs $state $credentials (Join-Path $case 'state.json') $target -Preview:($mode -eq 'preview') -Confirmed }
                catch { $failed = $true }
