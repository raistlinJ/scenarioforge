...
# ---------------------------------------------------------------------------
# Vuln grants carry no runtime value
# ---------------------------------------------------------------------------


def _validate_vuln_grant(monkeypatch, *, declares_input, upstream_producer):
    """Chain where a vuln grants WebAuthBypass(app) that a generator consumes."""
    consumer = {
        'plugin_id': 'consumer',
        'requires': ['WebAuthBypass(app)'],
        'produces': [{'artifact': 'Flag(flag_id)'}],
    }
    plugins = {'consumer': consumer}
    gen_defs = {
        'consumer': {
            'id': 'consumer',
            'inputs': (
                [{'name': 'WebAuthBypass(app)', 'required': True}] if declares_input else []
            ),
        }
    }
    chain_nodes = [{
        'id': 'h1', 'name': 'v1', 'type': 'docker',
        'is_vuln': True, 'vulnerabilities': [{'name': 'zz/CVE-BYPASS'}],
    }]
    assignments = [{'node_id': 'h1', 'id': 'consumer'}]

    if upstream_producer:
        plugins['producer'] = {
            'plugin_id': 'producer',
            'requires': [],
            'produces': [{'artifact': 'WebAuthBypass(app)'}, {'artifact': 'Flag(flag_id)'}],
        }
        gen_defs['producer'] = {'id': 'producer', 'inputs': []}
        chain_nodes.insert(0, {'id': 'h0', 'name': 'v0', 'type': 'docker'})
        assignments.insert(0, {'node_id': 'h0', 'id': 'producer'})

    monkeypatch.setattr(app_backend, '_flow_enabled_generator_defs_by_id', lambda: gen_defs)
    monkeypatch.setenv('SCENARIOFORGE_REQUIRE_VULN_METADATA', '0')
    _install_metadata(monkeypatch, [{'match': 'zz/CVE-BYPASS', 'impact': 'auth_bypass'}])
    return app_backend._flow_validate_chain_order_by_requires_produces(
        chain_nodes, assignments, scenario_label='zz-vuln-grant',
        plugins_by_id_override=plugins,
    )


def test_vuln_granted_fact_read_as_a_runtime_input_is_rejected(monkeypatch):
    """A vuln satisfies the solver but writes no value, so the input is fiction."""
    ok, errors = _validate_vuln_grant(monkeypatch, declares_input=True, upstream_producer=False)
    assert not ok
    assert any('only granted by a vulnerability' in e for e in errors), errors


def test_a_real_producer_upstream_makes_the_vuln_grant_fine(monkeypatch):
    """With a generator producing the fact, a real value reaches the consumer."""
    ok, errors = _validate_vuln_grant(monkeypatch, declares_input=True, upstream_producer=True)
    assert ok, errors


def test_vuln_granted_fact_not_consumed_as_an_input_stays_an_ordering_constraint(monkeypatch):
    """Requiring a fact without reading it needs no value, so the vuln grant suffices."""
    ok, errors = _validate_vuln_grant(monkeypatch, declares_input=False, upstream_producer=False)
    assert ok, errors
