...
def test_generator_builder_ai_messages_add_prompt_derived_ssh_credential_defaults():
    ...
    assert 'Suggested hint levels: low: Inspect the exposed service before moving to {{NEXT_NODE_NAME}}., medium: Credential: {{OUTPUT.Credential(user,password)}}, high: Answer: {{OUTPUT.Flag(flag_id)}}' in user_content
