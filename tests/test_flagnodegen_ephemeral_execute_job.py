...
def test_flagnodegen_build_ephemeral_execute_job_builds_flow_and_job_spec(tmp_path, monkeypatch):
    ...
    meta = {
        ...
        'generator_name': 'NFS Sensitive File',
        'core_cfg': {'ssh_host': '127.0.0.1', 'ssh_username': 'u', 'ssh_password': 'p', 'host': '127.0.0.1', 'port': 50051},
    }

    job_spec, err = app_backend._flagnodegen_build_ephemeral_execute_job(meta, run_id='abc123')
    ...
    assert assignment.get('type') == 'flag-node-generator'
    assert assignment.get('id') == 'nfs_sensitive_file'
    assert assignment.get('node_id') == 'd1'
    assert assignment.get('flag_value') == 'FLAG{demo-node}'
