...
host_command_available() {{
    case "$1" in
        qemu-img|xz|git|ssh|ssh-keygen|scp|timeout|sha256sum|sha512sum|gzip|xorriso|genisoimage) [[ "$installed" == 1 ]] ;;
...
''')
    ...
    install = calls[-1]
    for package in [qemu, xz, ssh, 'coreutils', 'gzip', 'xorriso', 'git', 'ca-certificates']:
