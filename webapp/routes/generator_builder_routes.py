...
def _build_builder_auto_heal_prompt(original_prompt: str, validation_errors: list[str]) -> str:
    ...
    lines = [
        ...
        '- If a runtime input is solver-facing, needed on sequence 1 or the first step of a parallel branch, and not reasonably discoverable by participants yet, mark it flow_supply_when_first.',
...
def _build_builder_scaffold_repair_prompt(original_prompt: str, scaffold_errors: list[str]) -> str:
    ...
    lines = [
        ...
        '- Mark solver-facing start-step runtime inputs with flow_supply_when_first when participants cannot reasonably discover them before solving sequence 1 or a parallel branch starter.',
        '- Include hint_levels.low, hint_levels.medium, and hint_levels.high with at least one non-empty hint each.',
...
def _build_generator_grounding_lines(plugin_type: str, *, compact: bool = False, ultra_compact: bool = False) -> list[str]:
    ...
    lines = [
        ...
        "- Write the low hint so it still reads correctly if the next-node pointer is removed: put the instruction first and any {{NEXT_NODE_*}} reference last, in its own clause. Flow drops that clause when nothing downstream consumes the generator output, so a hint whose entire body is a pointer degrades to a generic line.",
...
def _build_generator_builder_ai_messages(payload: dict[str, Any]) -> list[dict[str, str]]:
    ...
    kind_requirements = [
        ...
        '- For solver-facing runtime inputs that participants must use on sequence 1 or a parallel branch starter but cannot reasonably know yet, set flow_supply_when_first true so Flow supplies and hints them.',
    ...
    schema_lines = [
        ...
        '  "hint_levels": {"low": ["Inspect the exposed service before moving to {{NEXT_NODE_NAME}}."], "medium": ["Service/artifact: {{OUTPUT.File(path):basename}}"], "high": ["Use the access instructions and README.md for the complete workflow."]},',
    ...
    if ultra_compact_prompt:
        schema_lines = [
            '{"plugin_id":"source_identifier","folder_name":"py_source_identifier","name":"Human-readable name","description":"One sentence summary","requires":[{"artifact":"Knowledge(ip)","optional":false}],"optional_requires":[],"produces":["Flag(flag_id)"],"runtime_inputs":[],"hint_levels":{"low":["Inspect the exposed service before moving to {{NEXT_NODE_NAME}}."],"medium":["Artifact or service: {{OUTPUT.File(path)}}"],"high":["Use the access instructions and README.md for the complete workflow."]},"inject_files":[],"inject_candidate_paths":[],"access_instructions":{},"env":{},"readme_text":"full README.md text","generator_py_text":"full generator.py text","compose_text":"full docker-compose.yml text if needed"}',
        ]
