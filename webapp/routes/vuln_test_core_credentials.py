...
def register(
    ...
    *,
    ...
    """Register vuln-catalog test CORE SSH credential persistence routes.

    These routes back the "Save Credentials" toggle in the vuln catalog test/cache
    modal. Mirrors flag_generators_test_core_credentials so the two catalogs behave
    the same way (server-side persisted, not just localStorage).
    """

    if not begin_route_registration(app, 'vuln_test_core_credentials_routes'):
        return

    def _hint_path() -> str:
        return os.path.join(outputs_dir(), 'vuln_catalog_test_core_hint.json')
    ...
    def _save_view():
        ...
        secret_payload = {
            'scenario_name': 'vuln-catalog-test',
            'scenario_index': None,
            'grpc_host': grpc_host,
            'grpc_port': grpc_port,
            'ssh_host': ssh_host,
        ...
        try:
            ...
            try:
                if logger is not None:
                    logger.exception('[vulncat_test] failed to persist CORE creds: %s', exc)
            except Exception:
                pass
    ...
    app.add_url_rule(
        '/api/vuln_catalog_test/core_credentials/save',
        endpoint='api_vuln_catalog_test_core_credentials_save',
        view_func=_save_view,
        methods=['POST'],
    )
    app.add_url_rule(
        '/api/vuln_catalog_test/core_credentials/get',
        endpoint='api_vuln_catalog_test_core_credentials_get',
        view_func=_get_view,
        methods=['POST'],
    )
    mark_routes_registered(app, 'vuln_test_core_credentials_routes')
