...
- **Pivot access only opens what it must.** The "accessible by pivot" toggle exposes exactly one provider per walled-off subnet, on one port. It does not make the subnet generally reachable, and it never relaxes a block that already has a reachable provider behind it.
- **A pivot provider is never an unfilled challenge slot.** Selection falls back to a non-slot host, then the subnet's router, then a new node. If a subnet somehow has no host, no router, and node creation is disabled, the subnet is reported in `pivot_access.unresolved` with a reason rather than silently left unreachable.
...
- **An SSH fallback provider is a node built from an SSH image, not an existing node with the service switched on.** Node images are built offline-safe with no package manager, so a minimal image cannot grow an `sshd`. The planner pins `PIVOT_SSH_IMAGE` (override `CORETG_PIVOT_SSH_IMAGE`) and records it on the provider; the topology builder has to materialise that node, so until it does, a subnet serving nothing reports a provider with `added: true` and no address.
