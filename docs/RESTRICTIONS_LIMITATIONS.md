...
- **Ping is not a reachability test under default-deny segmentation.** A scenario that blocks subnets by default and allows specific service flows will drop ICMP while the flows themselves work. Flows are therefore tested on their own protocol and port; ping survives only as a follow-up diagnostic.
- **UDP delivery is confirmed at the destination, not by the sender.** UDP has no handshake, so a sent datagram proves only that the sender had a route. Confirmation comes from the receiving agent's byte counter; a flow with bytes sent and none received is reported as dropped in flight. When no agent counters are readable the flow reports `skip`, not `pass`. TCP flows are confirmed in both directions by the handshake itself.
- **Agent byte counters are cumulative.** They answer "has this traffic arrived" since the agent started, not "is it arriving right now". A flow that worked and later broke still shows its earlier bytes. Traffic patterns can idle by design (`periodic`, `burst`, `poisson`), so a short sampling window would produce false alarms — cumulative totals are deliberate.
...
- **Pivot access only opens what it must.** The "accessible by pivot" toggle exposes exactly one provider per walled-off subnet, on one port. It does not make the subnet generally reachable, and it never relaxes a block that already has a reachable provider behind it. That one port is open to `0.0.0.0/0`, because a provider is the subnet's entrance and who has to walk through it is not knowable from the rule that closed the subnet — the participant sits on a HITL link subnet that appears in no segmentation rule at all.
- **A pivot provider is never an unfilled challenge slot.** Selection prefers a node already offering a vulnerability, then a flag-node-generator, then SSH, and otherwise adds a Docker node. It never takes an empty slot, which would spend capacity the author reserved for challenges. When no provider can be placed — no switch serves the subnet, or it has no free address — the subnet is reported in `pivot_access.unresolved` with a reason rather than silently left unreachable.
- **Nested pivots are not supported.** A provider you could only reach by working through another provider is flattened: every provider is directly reachable, and the ordering between them is carried by the challenges (you need what the earlier step gave you) rather than by the network. Enforcing it at the network layer is not available — FORWARD allows go to every router by necessity, so there is no hop at which a later provider could be held back. Where a scenario's segmentation implies an ordering it is reported as `pivot_access.nested_candidates` alongside `nested_supported: false`.
- **Pivot providers are Docker-only, never vnodes.** Routers, PCs, servers and workstations are CORE vnodes, which share the CORE VM's filesystem — SSH on one is a host escape rather than a foothold in the scenario. A subnet whose only nodes are vnodes or unfilled challenge slots therefore needs a Docker node added.
- **An SSH fallback provider is a node built from an SSH image, not an existing node with the service switched on.** Node images are built offline-safe with no package manager, so a minimal image cannot grow an `sshd`. The planner pins `PIVOT_SSH_IMAGE` (override `CORETG_PIVOT_SSH_IMAGE`) and the port that image actually serves — **2222, not 22** (override `CORETG_PIVOT_SSH_PORT`). The node is allocated into the topology plan, so it exists by the end of the `topo` phase.
- **Segmentation and traffic are planned once and replayed.** Both planners draw from the global `random` module, so re-running either produces a different plan rather than the same one. Execute enforces the plan's rules and writes the plan's flows. A setting that reaches execute but not the plan (`--nat-mode` and friends) cannot be honoured; execute names it and uses the plan's value.
