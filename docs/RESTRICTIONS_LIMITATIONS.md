...
- **An SSH fallback provider needs an image with `sshd`.** Minimal container images ship neither `sshd` nor `iptables`; on such a node the pivot path opens to a closed port. Prefer a provider that already exposes a vulnerability or flag-node-generator.
